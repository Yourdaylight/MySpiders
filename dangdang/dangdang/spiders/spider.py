    ...
    def parse(self, response):
        ...
        for url,category in zip(urls,categories):#爬取分类链接的详情页
            ...
            for i in range(1,20):#爬取分页信息
                url=url+'&page_index={}#pos'.format(i)
                yield scrapy.Request(url=url,callback=lambda response, category=text : self.parse_subpage(response,category), dont_filter=True)
